...
var FlatTagView = class extends import_obsidian2.ItemView {
  ...
  async onOpen() {
    ...
    this.registerDomEvent(document, "keydown", (evt) => {
      ...
      if (((_a = this.app.workspace.activeLeaf) == null ? void 0 : _a.view) !== this)
        return;
      this.capsLockOn = (_c = (_b = evt.getModifierState) == null ? void 0 : _b.call(evt, "CapsLock")) != null ? _c : this.capsLockOn;
      if (evt.key === "Shift")
        this.shiftHeld = true;
      if (evt.key === "CapsLock")
        this.capsLockOn = (_e = (_d = evt.getModifierState) == null ? void 0 : _d.call(evt, "CapsLock")) != null ? _e : this.capsLockOn;
      const popupArmed = this.shiftHeld || this.capsLockOn;
      if (!popupArmed)
        return;
      if (this.hoveredTag) {
        ...
        this.hoverPreviewTimer = window.setTimeout(() => {
          ...
          if (!this.hoveredTag)
            return;
          if (this.lastPreviewTag === this.hoveredTag.tag)
            return;
          this.lastPreviewTag = this.hoveredTag.tag;
          this.showTagHoverPreview(this.hoveredTag.el, this.hoveredTag.tag);
        }, 80);
      }
    });
    ...
    this.registerDomEvent(document, "keydown", (evt) => {
      ...
      const caps = (_c = (_b = evt.getModifierState) == null ? void 0 : _b.call(evt, "CapsLock")) != null ? _c : false;
  ...
  positionHoverPreview(anchor, box, gap = 6, margin = 10) {
    ...
    if (available < h) {
      box.style.maxHeight = `${Math.max(80, available)}px`;
      box.style.overflowY = "auto";
    }
    const h2 = box.offsetHeight;
    let top = placeBelow ? a.bottom + gap : a.top - gap - h2;
    top = Math.max(margin, Math.min(top, vh - margin - h2));
    let left = a.left;
    if (left + w > vw - margin)
      left = a.right - w;
  ...
  showTagHoverPreview(anchor, tag) {
    this.clearHoverPreview();
    const box = document.createElement("div");
    box.className = "flat-tag-hover-preview";
    box.style.position = "fixed";
    box.style.zIndex = "9999";
    box.style.width = "max-content";
    box.style.maxWidth = "min(520px, calc(100vw - 20px))";
    box.addEventListener("mouseenter", () => this.hoverPreviewHovered = true);
    ...
    const matches = [];
    this.tagsByFile.forEach((tags, path) => {
      if (!tags.includes(tag))
        return;
      const af = this.app.vault.getAbstractFileByPath(path);
      if (af instanceof import_obsidian2.TFile)
        matches.push({ file: af, folder: this.folderLabelFromPath(af.path) });
    });
    matches.sort((a, b) => b.file.stat.mtime - a.file.stat.mtime);
    const top5 = matches.slice(0, 5);
    box.textContent = "";
    const title = document.createElement("div");
    title.className = "flat-tag-hover-preview-title";
    title.textContent = `#${tag} \u2014 newest`;
    box.appendChild(title);
    for (const m of top5) {
      const row = document.createElement("div");
      row.className = "flat-tag-hover-preview-row";
      const folder = document.createElement("span");
      folder.className = "flat-tag-hover-preview-folder";
      folder.textContent = m.folder;
      const sep = document.createElement("span");
      sep.className = "flat-tag-hover-preview-sep";
      sep.textContent = "|";
      const name = document.createElement("span");
      name.className = "flat-tag-hover-preview-name";
      name.textContent = m.file.name;
      row.append(folder, sep, name);
      row.addEventListener("click", () => {
        this.app.workspace.getLeaf(true).openFile(m.file);
        this.clearHoverPreview();
      });
      box.appendChild(row);
    }
    this.positionHoverPreview(anchor, box);
  }
  createTagElement(tag, count, parentEl) {
    ...
    tagEl.addEventListener("mouseenter", (e) => {
      ...
      if (import_obsidian2.Platform.isMobile)
        return;
      this.capsLockOn = (_b = (_a2 = e.getModifierState) == null ? void 0 : _a2.call(e, "CapsLock")) != null ? _b : this.capsLockOn;
      this.hoveredTag = { el: tagEl, tag };
      const popupArmed = this.shiftHeld || this.capsLockOn || e.shiftKey;
      if (!popupArmed)
        return;
      if (this.hoverPreviewTimer)
        window.clearTimeout(this.hoverPreviewTimer);
      this.hoverPreviewTimer = window.setTimeout(() => {
        if (!(this.shiftHeld || this.capsLockOn))
          return;
        if (!this.hoveredTag || this.hoveredTag.el !== tagEl)
          return;
        if (this.lastPreviewTag === tag)
          return;
        this.lastPreviewTag = tag;
        this.showTagHoverPreview(tagEl, tag);
      }, 80);
    });
    tagEl.addEventListener("mouseleave", (e) => {
      var _a2;
      if (((_a2 = this.hoveredTag) == null ? void 0 : _a2.el) === tagEl)
        this.hoveredTag = null;
      if (this.hoverPreviewTimer)
        window.clearTimeout(this.hoverPreviewTimer);
      this.hoverPreviewTimer = null;
      const rel = e.relatedTarget;
      if (this.hoverPreviewEl && rel && this.hoverPreviewEl.contains(rel))
        return;
      if (this.shiftHeld || this.capsLockOn)
        return;
      this.clearHoverPreview();
...
.flat-tag-hover-preview-title {
  margin-bottom: 6px;
  opacity: 0.85;
  font-size: 12px;
}

.flat-tag-hover-preview-row {
  display: grid;
  grid-template-columns: max-content 14px 1fr; /* folder | filename */
  column-gap: 6px;
  white-space: nowrap;
  cursor: pointer;
}

.flat-tag-hover-preview-folder { text-align: right; opacity: 0.8; }
.flat-tag-hover-preview-sep { text-align: center; opacity: 0.6; }
.flat-tag-hover-preview-name { text-align: left; }
